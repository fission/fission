...
func provisionWarmFunctionNamed(ctx context.Context, sc *harness.Scope, prefix string, executor fv1.ExecutorType, runtime warmRuntime, poolsize, requestsPerPod int, methods []string) (route, fnName string, err error) {
	...
	// GracePeriod matters: an Environment created via the API without it gets
	// terminationGracePeriodSeconds 0 — pods are SIGKILLed the instant a roll
	// or reap deletes them, and every in-flight request on them dies as a
	// connection reset. 90s mirrors the chart's router posture (75s drain +
	// margin) and covers the 60s default function timeout.

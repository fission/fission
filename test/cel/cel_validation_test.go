...
// enum/range scalar rules, SSA immutability, and the Package literal size cap.
//
...
func TestCELPackageLiteralSize(t *testing.T) {
	fc := startEnv(t)

	pkg := func(name string, literal []byte) *fv1.Package {
		return &fv1.Package{
			ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: ns},
			Spec: fv1.PackageSpec{
				Environment: fv1.EnvironmentReference{Name: "env", Namespace: ns},
				Deployment:  fv1.Archive{Type: fv1.ArchiveTypeLiteral, Literal: literal},
			},
		}
	}

	t.Run("literal at limit accepted", func(t *testing.T) {
		_, err := fc.CoreV1().Packages(ns).Create(t.Context(), pkg("p-ok", make([]byte, 262144)), metav1.CreateOptions{})
		assert.NoError(t, err)
	})

	t.Run("literal over 256KiB rejected", func(t *testing.T) {
		_, err := fc.CoreV1().Packages(ns).Create(t.Context(), pkg("p-too-big", make([]byte, 262145)), metav1.CreateOptions{})
		require.Error(t, err, "apiserver should reject oversize literal")
		assert.Contains(t, err.Error(), "256 kB")
	})
}

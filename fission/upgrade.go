...
func upgradeDumpState(c *cli.Context) error {
	u := lib.GetV1URL(c.GlobalString("server"))
	filename := c.String("file")

	// check v1
	resp, err := http.Get(u + "/environments")
	if err != nil {
		return lib.FailedToError(err, "reach fission server")
	}
	if resp.StatusCode == http.StatusNotFound {
		msg := fmt.Sprintf("Server %v isn't a v1 Fission server. Use --server to point at a pre-0.2.x Fission server.", u)
		return lib.GeneralError(msg)
	}

	lib.UpgradeDumpV1State(u, filename)
	return nil
}
...
func upgradeRestoreState(c *cli.Context) error {
	...
	if err != nil {
		return lib.FailedToError(err, fmt.Sprintf("open file %v", filename))
	}

	var v1state lib.V1FissionState
	err = json.Unmarshal(contents, &v1state)
	if err != nil {
		return lib.FailedToError(err, "parse dumped v1 state")
	}
	...
	for _, f := range v1state.Functions {
		...
		if err != nil {
			return lib.FailedToError(err, "create temporary file")
		}
		code, err := base64.StdEncoding.DecodeString(f.Code)
		if err != nil {
			return lib.FailedToError(err, "decode base64 function contents")
		}
		tmpfile.Write(code)
		tmpfile.Sync()
		tmpfile.Close()
		...
		// upload
		archive, err := lib.CreateArchive(client, tmpfile.Name(), "")
		if err != nil {
			return lib.FailedToError(err, fmt.Sprintf("create archive for function %s", fnName))
		}
		...
		if err != nil {
			return lib.FailedToError(err, fmt.Sprintf("create package %v", pkgName))
		}
		_, err = client.FunctionCreate(&crd.Function{
			Metadata: *lib.CrdMetadataFromV1Metadata(&f.Metadata, v1state.NameChanges),
		...
		if err != nil {
			return lib.FailedToError(err, fmt.Sprintf("create function %v", v1state.NameChanges[f.Metadata.Name]))
		}
	...
	for _, e := range v1state.Environments {
		_, err = client.EnvironmentCreate(&crd.Environment{
			Metadata: *lib.CrdMetadataFromV1Metadata(&e.Metadata, v1state.NameChanges),
		...
		if err != nil {
			return lib.FailedToError(err, fmt.Sprintf("create environment %v", e.Metadata.Name))
		}
	}
	...
	for _, t := range v1state.HTTPTriggers {
		_, err = client.HTTPTriggerCreate(&crd.HTTPTrigger{
			Metadata: *lib.CrdMetadataFromV1Metadata(&t.Metadata, v1state.NameChanges),
			Spec: fission.HTTPTriggerSpec{
				RelativeURL:       t.UrlPattern,
				Method:            t.Method,
				FunctionReference: *lib.FunctionRefFromV1Metadata(&t.Function, v1state.NameChanges),
			},
		})
		if err != nil {
			return lib.FailedToError(err, fmt.Sprintf("create http trigger %v", t.Metadata.Name))
		}
	}
	...
	for _, t := range v1state.TimeTriggers {
		...
		if err != nil {
			return lib.FailedToError(err, fmt.Sprintf("create time trigger %v", t.Metadata.Name))
		}
	}
	...
	for _, t := range v1state.Watches {
		...
		if err != nil {
			return lib.FailedToError(err, fmt.Sprintf("create kubernetes watch trigger %v", t.Metadata.Name))
		}
	}

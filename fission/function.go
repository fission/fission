...
func fnLogs(c *cli.Context) error {
	...
	respBody, err := client.FunctionLogsGet(fnName, c.String("pod"), c.Bool("d"), c.Bool("f"))
	defer respBody.Close()
	checkErr(err, "get function logs")
...
func fnPods(c *cli.Context) error {
	...
	respBody, err := client.FunctionPodsGet(fnName)
	defer respBody.Close()
	checkErr(err, "failed to get function pods")

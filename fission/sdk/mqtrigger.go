...
func MqtCreateCli(c *cli.Context) error {
...
func MqtGetCli(c *cli.Context) error {
	return nil
}

func MqtUpdateCli(c *cli.Context) error {
	client := GetClient(c.GlobalString("server"))
	mqtName := c.String("name")
	if len(mqtName) == 0 {
...
func MqtDeleteCli(c *cli.Context) error {
	client := GetClient(c.GlobalString("server"))
	mqtName := c.String("name")
	if len(mqtName) == 0 {
...
func MqtListCli(c *cli.Context) error {
	client := GetClient(c.GlobalString("server"))
	mqtNs := c.String("triggerns")

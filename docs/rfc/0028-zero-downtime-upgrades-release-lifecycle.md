...
- Status: Implemented, gate not yet passing ([#3631](https://github.com/fission/fission/pull/3631) phase 1, [#3636](https://github.com/fission/fission/pull/3636) phase 2, [#3637](https://github.com/fission/fission/pull/3637) phase 3, [#3641](https://github.com/fission/fission/pull/3641)/[#3644](https://github.com/fission/fission/pull/3644) phase 4, [#3642](https://github.com/fission/fission/pull/3642) phase 5, [#3649](https://github.com/fission/fission/pull/3649) follow-ups; merged 2026-07-31–2026-08-03) — all five phases shipped: the `RELEASES.md` lifecycle policy and the **upgrade-under-load CI leg** (install latest published release, drive constant-rate load, `helm upgrade` to HEAD mid-load, classify every outcome); rollout posture (router/webhook replicas + PDBs guarded on the autoscaler's **minReplicas**, graceful-shutdown wiring via `GRACEFUL_SHUTDOWN_TIMEOUT`, webhook cert stability); version-skew hardening (builder **Deployments** rolled on a pod-spec hash — [#776](https://github.com/fission/fission/issues/776), closed — plus auth-material skew and `fission check` skew reporting against the support window); the state-schema expand/contract contract with destructive-DDL, ordering and per-migration-checksum guards plus an upgrade-from-each-prior-schema conformance test; and a skip-level (N−2 → N) leg, exercised but deliberately not promised.
  **The gate measures and does not yet pass.** Latest `main`, ~3.6k requests per executor through a 31s upgrade: poolmgr **146 transport failures (4.0%)** + 6 platform-attributed, newdeploy 0 transport + **32 platform-attributed (0.9%)**, unattributed 0 on both, **warm-pod survival 0 of 1**. The published budget is zero on all six failure metrics, so the evaluation step stays `continue-on-error: true`. Driving poolmgr transport failures and warm-pod survival to zero, then removing that flag, is what closes [#1856](https://github.com/fission/fission/issues/1856) — the RFC landing does not.

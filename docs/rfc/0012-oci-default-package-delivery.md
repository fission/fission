...
- **Per-image pools default to poolsize 1** (the proposed Gate C contingency, applied by default): the env's poolsize multiplies per PACKAGE for per-image pools, and the first CI run with the producer on demonstrated the saturation concretely; warm depth stays the generic pool's job, kubelet-cached recreation keeps per-image cold starts cheap.
- **Push keychain skips ServiceAccount resolution** (`oci.NoServiceAccount`): the builder pod runs as `fission-builder`, which has no RBAC on the `fission-fetcher` SA the pull keychain resolves — and SA imagePullSecrets are a pull concept; pushes authenticate via the explicit push secret only.

...
| [0021](0021-statestore-substrate.md) | Statestore — Standard Durable-State Interface | Proposed: `pkg/statestore` KVStore/EventLog/Queue interfaces; external (user-managed Postgres) + embedded (Fission-owned SQLite) modes — Fission never ships a database; shared substrate for 0022/0023/0024 |
| [0022](0022-durable-function-workflows.md) | Durable Function Workflows | Proposed: `Workflow`/`WorkflowRun` CRDs, EventLog-fold engine (CAS-append, no leader election), Step Functions-style orchestration of functions; TLA+-checked protocol |

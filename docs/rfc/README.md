...
| [0018](0018-local-development-inner-loop.md) | Local-Development Inner Loop (`fission function run`) | Partially implemented (phases 0–1: interpreted one-shot local Docker loop — runtime image → `/v2/specialize` → invoke → teardown, cluster-less with `--image`); `--watch`, builder leg, fidelity bridges, and `--remote` remain phases 2–5 |

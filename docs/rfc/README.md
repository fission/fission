...
| [0019](0019-unified-opentelemetry-observability.md) | Unified OpenTelemetry Observability | Proposed: migrate the 39 metrics from Prometheus `client_golang` to the OTel Metrics API behind the OTel→Prometheus bridge exporter (byte-identical `/metrics`), add native OTLP metric push + trace exemplars, and trim the OTel footprint (drop `autoprop`, adopt `autoexport`, bump `semconv`, unify the provider bootstrap) |

...
(* subscriptions only tightens the reaper's trim bound); read-page shape   *)
(* (Deliver is per event: a consumer's Read batch, and the byte-bounded    *)
(* page of the optional BoundedEventLog / the statesvc eventlog read       *)
(* route, is a batching detail — every page carries >= 1 event and         *)
(* consumers stop on an EMPTY page, never on a short one, so paging by     *)
(* Seq makes progress and no event is skipped; a functional property       *)
(* covered by the driver conformance suite, not a concurrency claim).      *)
(*                                                                         *)

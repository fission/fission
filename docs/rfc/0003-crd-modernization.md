...
- Status: Partially implemented — k8s 1.32 floor ([#3448](https://github.com/fission/fission/pull/3448)), CEL validation + SSA markers ([#3449](https://github.com/fission/fission/pull/3449)), webhook trim for HTTPTrigger/TimeTrigger/CanaryConfig ([#3452](https://github.com/fission/fission/pull/3452)).
  Full webhook removal is out of reach: cross-namespace references, pod-spec validation (CEL cost budget), and byte-format archive fields cannot be expressed in CEL, so the Function/Package/Environment webhooks stay.
- Tracking issue: TBD
- Supersedes: —

...
- Status: Implemented (phases 0–2 and 4; phase 3 footprint cuts except the `autoexport` bootstrap consolidation, deferred).
  Fission's tracing and logging already spoke OpenTelemetry; **metrics were the gap** — all 39 metric definitions were hand-built on the Prometheus `client_golang` API, with no OTel metrics SDK in the process.
  This RFC migrates the metrics *instrumentation* to the OpenTelemetry Metrics API behind the OTel→Prometheus **bridge exporter**, so the `/metrics` endpoint, every metric name, label, type, and histogram bucket stay scrape-compatible (existing dashboards, alerts, ServiceMonitors keep working) while the platform gains an opt-in native OTLP metric push and trace exemplars on the request-path histograms.
  It also trims the OpenTelemetry dependency and runtime footprint (dropped `autoprop` and the four propagator modules it pulled, bumped `semconv` v1.4.0→v1.41.0).
  The `autoexport`-based bootstrap consolidation is **deferred** to a follow-up: `autoexport` defaults to an OTLP exporter when `OTEL_TRACES_EXPORTER` is unset, which would change today's "no endpoint = no tracing" behavior, so the hand-rolled exporter constructors are kept until that default is handled carefully.
- Tracking issue: —
...
- Requires: the OpenTelemetry Go **metrics** packages — `go.opentelemetry.io/otel/metric` (promoted from indirect to direct), `go.opentelemetry.io/otel/sdk/metric`, `go.opentelemetry.io/otel/exporters/prometheus` (v0.66.0, which pulls `github.com/prometheus/otlptranslator`), and `go.opentelemetry.io/otel/exporters/otlp/otlpmetric/otlpmetricgrpc`.
  No Kubernetes floor change; no new **bundled** runtime dependency.
  Net dependency change: +5 metrics modules, −5 propagator/auto modules (`autoprop` + aws/b3/jaeger/ot), so the module count is roughly flat while the *default* propagator surface shrinks.
...
The guiding constraint is backward compatibility: existing Grafana dashboards, alerts, ServiceMonitors, and PodMonitors must keep working untouched, enforced by a parity test covering every instrument kind.
...
- Keep the `/metrics` endpoint **scrape-compatible**: same port (8080), names, label keys and values, metric types, histogram buckets, and HELP text — enforced by a parity test.
...
- **Reduce the OpenTelemetry footprint**: an explicit W3C propagator (no `autoprop`, dropping its four propagator modules) and current `semconv`; an env-driven `autoexport` bootstrap is the deferred follow-up.
...
**Exposition stays Prometheus, scrape-compatible.**
Register the OTel Prometheus exporter against the *existing* registry rather than a new one (the exporter v0.66.0 API; `WithoutUnits`/`WithoutCounterSuffixes` were deprecated in favor of the translation strategy):

```go
promExporter, err := otelprom.New(
    otelprom.WithRegisterer(metrics.Registry), // the same registry ServeMetrics already serves
    otelprom.WithTranslationStrategy(otlptranslator.UnderscoreEscapingWithoutSuffixes), // no unit/_total suffixing; names verbatim
...
"Scrape-compatible" means the metric names, label keys/values, types, and histogram bucket boundaries are preserved — what dashboards and alerts query — not necessarily a byte-for-byte text match (OpenMetrics `_created` lines and HELP ordering may differ).
...
|---|---|---|
| `CounterVec` (names end `_total`) | `Int64Counter` | keep literal name incl. `_total`; the without-suffixes strategy prevents `_total_total` |
| `Gauge`/`GaugeVec` (inc/dec) | `Int64UpDownCounter` | exposed as a Prometheus gauge; balances naturally (no `Delete`/`Reset` exists today) |
| `Gauge`/`GaugeVec` (set to a value) | `Int64Gauge` (synchronous, available since SDK v1.28) | direct map of `.Set()` |
| `GaugeFunc` (e.g. `..._endpointcache_size`) | `Int64ObservableGauge` with callback | the callback reads the live value each collection |
| `HistogramVec` | `Float64Histogram` + `WithExplicitBucketBoundaries` | buckets must be pinned — see gotcha |
...
Each histogram is pinned at creation with `metric.WithExplicitBucketBoundaries(...)`, passing the same boundaries verbatim:
...
The instrument option is simpler than a reader View and replays correctly through the global provider's delegation; the parity test asserts the resulting `_bucket` `le` series match.
...
- **Drop `autoprop` for an explicit W3C composite (done).**
  `autoprop.NewTextMapPropagator()` is replaced with `propagation.NewCompositeTextMapPropagator(propagation.TraceContext{}, propagation.Baggage{})`.
...
- **Bump `semconv` v1.4.0 → v1.41.0 (done).**
  Refreshes the resource conventions; `semconv.ServiceNameKey` is unchanged so the bump is a pure import swap here.
- **`autoexport` bootstrap consolidation (deferred).**
  The intent is to collapse `getTraceExporter`/`getLogExporter`/the metric reader into `autoexport.NewSpanExporter`/`NewMetricReader`/the log equivalent, env-driven (`OTEL_*_EXPORTER`, `OTEL_EXPORTER_OTLP_PROTOCOL`).
  Deferred because `autoexport` defaults to an OTLP exporter when `OTEL_TRACES_EXPORTER` is unset, which would change today's "no endpoint = no tracing" behavior; the hand-rolled constructors stay until that default is handled.
- **`otel/log/logtest` stays a direct require.**
  It is imported by `loggerfactory_test.go`; Go has no separate test-require section, so a test-imported module is legitimately a direct require.
  No change.

A note of honesty on "footprint": the chosen metrics direction *adds* the OTel metrics SDK and the Prometheus/OTLP metric exporters, which the pure-Prometheus status quo did not carry, while dropping `autoprop` and its four propagator modules.
The net module count is roughly flat; the real win is **fewer default propagator modules**, **less custom propagator code**, and an opt-in **native OTLP metric path** the platform did not have before.
...
- **`/metrics` is scrape-compatible.**
  Same endpoint, port, names, label keys/values, metric types, histogram buckets, and HELP text.
  The exporter options (`WithTranslationStrategy(UnderscoreEscapingWithoutSuffixes)`, `WithoutScopeInfo`, `WithoutTargetInfo`) plus explicit bucket boundaries plus literal instrument names guarantee it, and a **parity test** locks it: gather the migrated registry and assert each metric's name, type, labels, and bucket boundaries.
...
- **Phase 0 — meter-provider scaffold (done).**
  Added the `MeterProvider` with the Prometheus bridge exporter bound to the existing registry, installed by `InitProvider`; the spike migrated `http_metrics` and proved the global-provider delegation wires package-init instruments.
- **Phase 1 — first subsystem, prove the pattern (done).**
  Router migrated to the meter helper; parity test asserts names/types/buckets; the `fission_function_overhead_seconds` histogram records with the request context so it carries exemplars.
- **Phase 2 — migrate the rest (done).**
  Executor (+client), storagesvc, mqtrigger, buildermgr, endpointcache, the generic `http_requests_*`, and the error-sampler metrics.
- **Phase 3 — footprint cuts (partial).**
  Dropped `autoprop` for the explicit W3C composite and bumped `semconv`; the `autoexport` bootstrap consolidation is deferred (see Pillar 2).
- **Phase 4 — native OTLP metric push (done).**
  An OTLP periodic reader attaches behind `OTEL_METRICS_EXPORTER`/`openTelemetry.metricsExporter`, alongside (never replacing) the Prometheus scrape.
- **Follow-up.**
  `autoexport` consolidation; per-subsystem golden `/metrics` capture tests; updating `test/integration/otel/` to exercise direct OTLP metric push.
...
- **Metric-name drift from the exporter's default suffixing.**
  ...
  Mitigated by the explicit exporter options above and caught by the parity test.
- **Histogram bucket divergence.**
  OTel's default aggregation is not Prometheus buckets.
  Mitigated by mandatory explicit bucket boundaries for all three histograms; the parity test asserts the `_bucket` boundaries.

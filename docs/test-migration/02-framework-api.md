...
Cached fields: typed Fission clientset, Kubernetes clientset, runtime image registry, the portless route registry (router/router-internal/mcp), logger.
...
HTTP client targeting the Fission router. The base URL host is a portless route name resolved by the client's transport — an in-process port-forward to `svc/router` by default, or the `FISSION_ROUTER` override address.

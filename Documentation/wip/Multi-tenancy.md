...
It mandates that a function reference secrets, configmaps and its package (if explicitly referenced during function creation/updation) to be present in the same namespace as the function.
...
Next when the user creates a function in the same namespace, if the function's executor type is newdeploy, then, the fission-fetcher SA present in the function namespace is also added to the same rolebinding.

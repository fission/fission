...
func (c *Cache) service() {
	for {
		...
		case GET:
			...
			if !ok {
				resp.error = ferror.MakeError(types.ErrorNotFound,
		...
		case SET:
			...
			if _, ok := c.cache[req.key]; ok {
				...
				resp.error = ferror.MakeError(types.ErrorNameExists, "key already exists")
			} else {
		...
		case COPY:
			...
			req.responseChannel <- resp
		default:
			resp.error = ferror.MakeError(types.ErrorInvalidArgument,

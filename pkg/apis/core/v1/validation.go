...
// needs a parser), so the admission webhook calls ValidateForAdmission; the
...
const (
	...
	// that also keeps the reachability walk trivially cheap.
...
func (spec WorkflowSpec) Validate() error {
	...
	for name, st := range spec.States {
		errs = errors.Join(errs, st.validate(fmt.Sprintf("WorkflowSpec.States[%s]", name), spec.States))
	}
...
// validateWorkflowRetry bounds a workflow retry policy. Deliberately not
// InvocationConfig.Validate: async delivery attempts are statestore-queue
// deliveries clamped to MaxAsyncAttempts, workflow attempts are fold-level.
...
// ValidateForAdmission is what the admission webhook enforces: the full rule
// set (nothing here is CEL-expressible — reachability needs a traversal,
// JSONPath needs a parser).
func (w *Workflow) ValidateForAdmission() error {
	return w.Validate()
}
...
// ValidateForAdmission mirrors Validate: the input byte cap cannot be
// expressed in CEL (raw-bytes fields break CEL cost estimation).
func (wr *WorkflowRun) ValidateForAdmission() error {
	return wr.Validate()
}

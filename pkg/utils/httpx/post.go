...
// errors with the same exponential backoff the in-cluster specialize path uses
// — for talking to a server that may not have finished starting (e.g. an env
// container's /v2/specialize right after the container turns ready). It returns
// nil on a 2xx response and the last error otherwise. onAttempt, when non-nil,
// is called before each attempt (used to emit an OTel span event in-cluster).
func PostWithConnRetry(ctx context.Context, client *http.Client, url, contentType string, body []byte, logger logr.Logger, maxRetries int, onAttempt func()) error {
	...
	for i := range maxRetries {
		...
		if netErr != nil && (netErr.IsConnRefusedError() || netErr.IsDialError()) && i < maxRetries-1 {

...
var (
	...
	MqtPollingInterval = Flag{Type: Int, Name: flagkey.MqtPollingInterval, Usage: "Interval to check each trigger source", DefaultValue: 30}
	MqtCooldownPeriod  = Flag{Type: Int, Name: flagkey.MqtCooldownPeriod, Usage: "The period to wait after the last trigger reported active before scaling the deployment back to 0", DefaultValue: 300}
	MqtMinReplicaCount = Flag{Type: Int, Name: flagkey.MqtMinReplicaCount, Usage: "Minimum number of replicas KEDA will scale the deployment down to", DefaultValue: 0}
	MqtMaxReplicaCount = Flag{Type: Int, Name: flagkey.MqtMaxReplicaCount, Usage: "Maximum number of replicas KEDA will scale the deployment down to", DefaultValue: 100}
	MqtMetadata        = Flag{Type: StringSlice, Name: flagkey.MqtMetadata, Usage: "KEDA Scaler trigger specification parameters: --metadata key1=value1 --metadata key2=value2"}

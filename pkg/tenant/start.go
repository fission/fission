...
func Start(ctx context.Context, clientGen crd.ClientGeneratorInterface, logger logr.Logger, _ *errgroup.Group) error {
	...
	if err := controller.Register(crMgr, &fv1.FissionTenant{},
		&TenantReconciler{logger: logger.WithName("tenant"), client: crMgr.GetClient(), resolver: resolver},
		"fission-tenant"); err != nil {

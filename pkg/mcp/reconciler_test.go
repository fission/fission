...
func TestFunctionToolReconcilerNotExposed(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Empty(t, reg.ListForNamespaces(nil, true), "a function without Tool must not be advertised")
}

func TestFunctionToolReconcilerToggleOff(t *testing.T) {
	fn := exposedFn("toggle", &fv1.ToolConfig{ExposeAsMCP: true, Description: "d"})
	r, reg, c := newReconciler(t, fn)
	...
	// Flip ExposeAsMCP off and reconcile: the tool must be dropped.
	got := &fv1.Function{}
	require.NoError(t, c.Get(ctx, key, got))
	got.Spec.Tool.ExposeAsMCP = false
	require.NoError(t, c.Update(ctx, got))

	_, err = r.Reconcile(ctx, ctrl.Request{NamespacedName: key})
	require.NoError(t, err)
	_, ok = reg.Lookup("default-toggle")
	assert.False(t, ok, "tool should be removed once ExposeAsMCP is false")
}

func TestFunctionToolReconcilerRename(t *testing.T) {
	fn := exposedFn("rn", &fv1.ToolConfig{ExposeAsMCP: true, Description: "d", ToolName: "first"})
	r, reg, c := newReconciler(t, fn)
...
func TestFunctionToolReconcilerDelete(t *testing.T) {
	fn := exposedFn("del", &fv1.ToolConfig{ExposeAsMCP: true, Description: "d"})
	r, reg, c := newReconciler(t, fn)
	...
	_, err := r.Reconcile(ctx, ctrl.Request{NamespacedName: key})
	require.NoError(t, err)
	require.NotEmpty(t, reg.ListForNamespaces(nil, true))
	...
	_, err = r.Reconcile(ctx, ctrl.Request{NamespacedName: key})
	require.NoError(t, err)
	assert.Empty(t, reg.ListForNamespaces(nil, true), "tool removed after function deletion")
}

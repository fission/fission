...
// condition shape. Best-effort: status I/O failures are logged at V(1) and
// never propagated back to the caller — the executor's primary job is to
// return a usable service URL, not to gate on status writes.
...
// signal "at least one runtime pod is ready in the pool" (Reason=PoolReady),
// which is a stronger readiness signal than the buildermgr's BuilderReady.
// Best-effort, same semantics as SetFunctionReady.

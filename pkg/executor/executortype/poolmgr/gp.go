...
func (gp *GenericPool) scheduleDeletePod(name string) {
	go func() {
		...
		err := gp.kubernetesClient.CoreV1().Pods(gp.namespace).Delete(context.Background(), name, metav1.DeleteOptions{})
		if err != nil {
			gp.logger.Error(
				"error deleting pod",
...
func (gp *GenericPool) getFuncSvc(ctx context.Context, fn *fv1.Function) (*fscache.FuncSvc, error) {
	...
	if gp.useIstio {
		...
		for _, pod := range podList.Items {
			...
			gp.kubernetesClient.CoreV1().Pods(gp.namespace).Delete(context.Background(), pod.ObjectMeta.Name, metav1.DeleteOptions{}) //nolint errcheck
		}
	}

	...
	for _, cm := range fn.Spec.ConfigMaps {
		...
		if err != nil {
			if k8s_err.IsNotFound(err) {
				...
				return fmt.Errorf("%s", fmt.Sprintf("configmap %s must be in same namespace as function namespace", cm.Name))
			} else {
				return err
			}
		}
		...
		if err != nil {
			if k8s_err.IsNotFound(err) {
				logger.Error("secret namespace mismatch", zap.String("error", "secret must be in same namespace as function namespace"),
					...
					zap.String("function_name", fn.ObjectMeta.Name),
					zap.String("function_namespace", gp.fnNamespace))
				return fmt.Errorf("%s", fmt.Sprintf("secret %s must be in same namespace as function namespace", sec.Name))
			} else {
				return err
			}

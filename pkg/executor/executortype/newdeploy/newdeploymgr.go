...
func (deploy *NewDeploy) RefreshFuncPods(logger *zap.Logger, f fv1.Function) error {
	...
	env, err := deploy.fissionClient.Environments(f.Spec.Environment.Namespace).Get(f.Spec.Environment.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	store, controller := k8sCache.NewInformer(listWatch, &fv1.Environment{}, resyncPeriod, k8sCache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(oldObj interface{}, newObj interface{}) {
			...
			if oldEnv.Spec.Runtime.Image != newEnv.Spec.Runtime.Image {
				...
				for _, f := range funcs {
					function, err := deploy.fissionClient.Functions(f.ObjectMeta.Namespace).Get(f.ObjectMeta.Name, metav1.GetOptions{})
					if err != nil {
...
func (deploy *NewDeploy) idleObjectReaper() {
	...
	for {
		...
		for _, fsvc := range funcSvcs {
			...
			fn, err := deploy.fissionClient.Functions(fsvc.Function.Namespace).Get(fsvc.Function.Name, metav1.GetOptions{})
			if err != nil {

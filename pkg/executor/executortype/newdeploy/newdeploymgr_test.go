...
func TestGetObjectReaperInterval(t *testing.T) {
	...
	want = 2
	os.Setenv("NEWDEPLOY_OBJECT_REAPER_INTERVAL", fmt.Sprint(want))
	...
	want = 3
	os.Unsetenv("NEWDEPLOY_OBJECT_REAPER_INTERVAL")
	...
	want = 4
	os.Setenv("NEWDEPLOY_OBJECT_REAPER_INTERVAL", "just some string!")
	os.Unsetenv("OBJECT_REAPER_INTERVAL")
	got = getObjectReaperInterval(logger, want)
	if want != got {
		t.Fatalf(`%d %d`, want, got)
	...
	want = 5
	os.Setenv("NEWDEPLOY_OBJECT_REAPER_INTERVAL", "")
	os.Unsetenv("OBJECT_REAPER_INTERVAL")
	got = getObjectReaperInterval(logger, 5)
	if want != got {
		t.Fatalf(`%d %d`, want, got)

...
import (
	...
	"github.com/fission/fission/pkg/fission/logdb"
...
func (api *API) ApiVersionMismatchHandler(w http.ResponseWriter, r *http.Request) {
	err := ferror.MakeError(types.ErrorNotFound, "Fission server supports API v2 only -- v1 is not supported. Please upgrade your Fission client/CLI.")
	api.respondWithError(w, err)
}

...
func (a *API) EnvironmentApiCreate(w http.ResponseWriter, r *http.Request) {
	...
	enew, err := a.fissionClient.Environments(env.ObjectMeta.Namespace).Create(&env)
	if err != nil {
		a.respondWithError(w, err)
		return
	}
...
func (a *API) EnvironmentApiGet(w http.ResponseWriter, r *http.Request) {
	...
	env, err := a.fissionClient.Environments(ns).Get(name, metav1.GetOptions{})
	if err != nil {
		a.respondWithError(w, err)
		return
	}
...
func (a *API) EnvironmentApiUpdate(w http.ResponseWriter, r *http.Request) {
	...
	enew, err := a.fissionClient.Environments(env.ObjectMeta.Namespace).Update(&env)
	if err != nil {
		a.respondWithError(w, err)
		return
	}

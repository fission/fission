...
func (a *API) MessageQueueTriggerApiCreate(w http.ResponseWriter, r *http.Request) {
	...
	tnew, err := a.fissionClient.MessageQueueTriggers(mqTrigger.ObjectMeta.Namespace).Create(&mqTrigger)
	if err != nil {
		a.respondWithError(w, err)
		return
	}
...
func (a *API) MessageQueueTriggerApiGet(w http.ResponseWriter, r *http.Request) {
	...
	mqTrigger, err := a.fissionClient.MessageQueueTriggers(ns).Get(name, metav1.GetOptions{})
	if err != nil {
		a.respondWithError(w, err)
		return
	}
...
func (a *API) MessageQueueTriggerApiUpdate(w http.ResponseWriter, r *http.Request) {
	...
	tnew, err := a.fissionClient.MessageQueueTriggers(mqTrigger.ObjectMeta.Namespace).Update(&mqTrigger)
	if err != nil {
		a.respondWithError(w, err)
		return
	}

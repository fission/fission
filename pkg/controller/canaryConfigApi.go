...
func (a *API) CanaryConfigApiCreate(w http.ResponseWriter, r *http.Request) {
	...
	canaryCfgNew, err := a.fissionClient.CanaryConfigs(canaryCfg.ObjectMeta.Namespace).Create(&canaryCfg)
	if err != nil {
		a.respondWithError(w, err)
		return
	}
...
func (a *API) CanaryConfigApiGet(w http.ResponseWriter, r *http.Request) {
	...
	canaryCfg, err := a.fissionClient.CanaryConfigs(ns).Get(name, metav1.GetOptions{})
	if err != nil {
		a.respondWithError(w, err)
		return
	}
...
func (a *API) CanaryConfigApiUpdate(w http.ResponseWriter, r *http.Request) {
	...
	canayCfgNew, err := a.fissionClient.CanaryConfigs(c.ObjectMeta.Namespace).Update(&c)
	if err != nil {
		a.respondWithError(w, err)
		return
	}

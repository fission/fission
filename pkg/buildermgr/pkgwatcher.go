...
func (pkgw *packageWatcher) build(ctx context.Context, srcpkg *fv1.Package) {
	...
	for healthCheckBackOff.NextExists() {
		...
		for _, item := range items {
			...
			err := utils.SetupRoleBinding(ctx, pkgw.logger, pkgw.k8sClient, fv1.PackageGetterRB, pkg.ObjectMeta.Namespace, utils.GetPackageGetterCR(), fv1.Role, fv1.FissionBuilderSA, builderNs)
			if err != nil {

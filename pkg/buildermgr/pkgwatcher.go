...
func (pkgw *packageWatcher) build(buildCache *cache.Cache, srcpkg *fv1.Package) {
	...
	env, err := pkgw.fissionClient.CoreV1().Environments(pkg.Spec.Environment.Namespace).Get(context.Background(), pkg.Spec.Environment.Name, metav1.GetOptions{})
	...
	for healthCheckBackOff.NextExists() {
		...
		for _, item := range items {
			...
			fnList, err := pkgw.fissionClient.CoreV1().
				Functions(metav1.NamespaceAll).List(context.Background(), metav1.ListOptions{})
			if err != nil {
			...
			for _, fn := range fnList.Items {
				if fn.Spec.Package.PackageRef.Name == pkg.ObjectMeta.Name &&
					...
					fn.Spec.Package.PackageRef.ResourceVersion = pkg.ObjectMeta.ResourceVersion
					// update CRD
					_, err = pkgw.fissionClient.CoreV1().Functions(fn.ObjectMeta.Namespace).Update(context.Background(), &fn, metav1.UpdateOptions{})
					if err != nil {
...
func setInitialBuildStatus(fissionClient *crd.FissionClient, pkg *fv1.Package) (*fv1.Package, error) {
	...
	return fissionClient.CoreV1().Packages(pkg.Namespace).Update(context.Background(), pkg, metav1.UpdateOptions{})
}

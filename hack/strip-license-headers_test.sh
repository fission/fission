...
# 4b. Go file with a //go:build prefix before the block (generated style).
{ echo '//go:build !ignore_autogenerated'; echo '// +build !ignore_autogenerated'; echo; \
  apache_go; } > "$tmp/gen.go"
python3 "$SCRIPT" "$tmp/gen.go"
head -1 "$tmp/gen.go" | grep -q '^//go:build !ignore_autogenerated$' || fail "build tag lost"
grep -q 'Apache License' "$tmp/gen.go" && fail "build-tag block not stripped" || true
grep -q 'func Bar' "$tmp/gen.go" || fail "build-tag body lost"

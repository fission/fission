...
Address resolution is behind `AddressResolver` (`pkg/router/resolver*.go`): `mode=shadow` compares executor answers against the index without routing changes; `mode=on` wires `fallbackResolver` (index admit → `/v2/ensureCapacity` on saturation → legacy RPC on miss/strict/degrade).
kind-ci runs gates on; one non-coverage CI leg pins them off post-deploy (the "Pin legacy data plane" step in `push_pr.yaml`) so the legacy path stays covered.
Phase 4 (defaults flip + dead-code deletion) is deferred to the next minor release.

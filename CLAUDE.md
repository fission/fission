...
`PropagateRequestCancellation` ties the handler ctx to the POST, so a client disconnect cancels an in-flight call (the proxy classifies it "canceled", not a function failure).
`AddTool` **panics** on an input schema whose `type` is not `"object"` — admission (`ToolConfig.Validate`) rejects those, so never relax that check.
